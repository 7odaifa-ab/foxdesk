    ...
    try {
        foreach ($pdo->query($sql) as $row) {
            ...
            foreach ($columns as $column) {
                ...
                $record = ['table' => $table, 'column' => $column, 'row' => (string) $row[$pk], 'score' => $score, 'repairable' => $candidate !== null];
                if ($showValues) {
...
$summary = ['mode' => $apply ? 'apply' : 'dry-run', 'scanned_rows' => $scannedRows, 'findings' => $findings, 'repairable' => $repairable, 'updated' => $updated, 'skipped_tables_without_single_primary_key' => $skippedTables];
