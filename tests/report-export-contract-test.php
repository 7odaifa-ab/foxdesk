...
$page = file_get_contents($root . '/includes/modules/reports/report-page-controller.php');
...
$assert(str_contains($page, 'report_query_time_entries($report_filter_state'), 'Reports controller must load entries through the report query module.');
$assert(str_contains($page, 'report_export_csv_if_requested($request'), 'Reports controller must delegate CSV export.');
...
foreach ([
    ...
    $assert(!str_contains($page, $needle), 'Reports controller must not own query/export logic: ' . $needle);
}
