...
$page = file_get_contents($root . '/includes/modules/reports/views/page.php');
$billing = file_get_contents($root . '/includes/modules/reports/views/billing.php');
$asset = file_get_contents($root . '/assets/js/report-billing-review.js');

$assert($page !== false && $billing !== false && $asset !== false, 'Report views and billing review JS asset must be readable.');
$assert(str_contains($page, 'assets/js/report-billing-review.js'), 'Reports page must load the extracted billing review JS asset.');
$assert(str_contains($billing, 'data-report-currency='), 'Reporting review surface must expose a currency data attribute.');
